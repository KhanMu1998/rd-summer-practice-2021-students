  ...
  (function (utils) {
    ...
    utils.canUserCancelGame = function (gameApi, gameInfo) {
      ...
      return (
        ...
        gameApi &&
        gameApi.questor &&
        gameApi.questor.user &&
        (gameApi.questor.user.isAdmin ||
          gameInfo.owner.id.toLowerCase() ===
            gameApi.questor.user.id.toLowerCase())
      );
    };
    ...
    utils.unpackMap = function (map) {
      ...
      for (i = 0; i < map.cells.length; i++) {
        ...
        if (
          cell.type !== GameApi.MapCellType.policeRespawn &&
  ...
  (function (gameList) {
    gameList.GameTable = (function () {
      ...
      GameTable.prototype.addRow = function (gameInfo, test) {
        ...
        if (
          app.utils.canUserCancelGame(this.gameApi, gameInfo) &&
          gameInfo.status !== GameApi.GameStatus.finished &&
          gameInfo.status !== GameApi.GameStatus.canceled
        ) {
      ...
      GameTable.prototype.load = function () {
        ...
        var callback = function (testGame, error) {
          ...
          this.gameApi.games.get(
            function (games, error) {
              if (!games && error) {
                this.showError();
              } else {
...
(function (app, $) {
  (function (newGame) {
    newGame.GameForm = (function () {
      var mapName =
        "{name} [{owner}] ({width}x{height}) {{policeCount}:{thiefCount}}";
    ...
    game.GameState = (function () {
      ...
      GameState.prototype.setTimer = function () {
        ...
        if (msSpend >= this.millisecondsToSwitch) {
          this.callbacks.timerChanged.fire(
            { m: 0, s: 0, total: 0 },
            this.switchTimeout
          );
          return false;
      ...
      GameState.prototype.getPlayer = function (id) {
        return this.teams && this.teams.team1 && this.teams.team2
          ? this.teams.team1.players[id] || this.teams.team2.players[id]
          : null;
      };
      GameState.prototype.removePlayerFromTeam = function (
        player,
        team,
        disconnected
      ) {
  ...
  (function (game) {
    game.GameView = (function () {
      ...
      GameView.prototype.bindEvents = function () {
        ...
        c.teamCaptionChanged.add(
          function (team) {
            this.updateTeamCaption(team);
          }.bind(this)
        );
        c.teamCoinsChanged.add(
          function (team) {
            this.updateTeamCoins(team);
          }.bind(this)
        );
        c.teamLivesChanged.add(
          function (team) {
            this.updateTeamLives(team);
          }.bind(this)
        );
        c.teamPlayersChanged.add(
          function (team) {
      ...
      GameView.prototype.drawPlayer = function (
        ...
        var img = police
          ? self
            ? this.imgs.imgPoliceSelf
            : this.imgs.imgPolice
          : self
          ? this.imgs.imgThiefSelf
          : this.imgs.imgThief;
        ctx.save();
      ...
      GameView.prototype.updateMap = function (map) {
        ...
        if (!this.game.mapBuffer) {
          ...
          this.game.mapBuffer = getCanvasBuffer(
            width,
            height,
            map,
            this.game.mapCellSize
          );
        }
      ...
      GameView.prototype.setGameCaption = function (name, status) {
         ...
         */
        this.game.$gameCaption.empty().append(
          $(
            app.utils.t(
                ...
                "<span class='game-caption-status game-caption-status-{status}'>{statusName}</span>",
              {
                name: name,
                status: status,
                statusName: app.utils.getStatusName(status),
              }
            )
          )
        );
      ...
      GameView.prototype.setTimer = function (data) {
        ...
        this.game.$switchTimer.empty().append(
          app.utils.t("<span class='{state}'>{m}:{s}</span>", {
            state: timerState,
            m: minutes,
            s: seconds,
          })
      ...
      GameView.prototype.setTeamCaption = function (team, $team) {
        ...
        var role =
          team.role === GameApi.GameTeamRole.police ? "police" : "thief";
        ...
        $team.$caption.empty().append(
          app.utils.t(
              ...
              "</td>",
            {
              role: role,
              roleName:
                team.role === GameApi.GameTeamRole.police
                  ? "полиция"
                  : "мошенники",
              name: team.name,
            }
      ...
      GameView.prototype.setButtons = function (status) {
        ...
        if (
          status === GameApi.GameStatus.canceled ||
          status === GameApi.GameStatus.finished
        ) {
          //если игра закрыта или окончена
